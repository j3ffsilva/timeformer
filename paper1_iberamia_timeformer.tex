...
We introduce a controlled diagnostic suite with known ground-truth semantic
trajectories, and formalize Timeformer, a family of joint token-time
conditioning designs. Token-Time conditioning more than doubles neighborhood
drift for drifting subjects relative to Standard; in this high-fidelity setting
a global temporal signal (Additive control) reproduces the aggregate drift, and
a reproducible advantage of joint conditioning appears only when local markers
are removed (ambiguous split, probe accuracy 0.595 vs.\ 0.579). The
Memory-Augmented extension exposes a second failure mode: when a word develops
coexisting senses, mean pooling collapses both into a centroid that represents
neither.
...
effect to architecture rather than data. The controlled diagnostic suite we
introduce isolates this effect before moving to open-domain corpora.

Against this diagnostic suite we formalize Timeformer, a family of joint
token-time conditioning designs. The central comparison is Standard
vs.\ Token-Time: does joint token-period conditioning improve temporal
traceability? Additive (global period shift, no token-level binding) serves as
a secondary control; adjudication between conditioning forms across noisier
regimes is left for future work. Memory-Augmented then tests whether causal history adds anything further.

The paper makes five contributions: (1)~it establishes temporal traceability
as a geometric property distinct from period-label recoverability;
(2)~it proposes Timeformer, a family of joint token-time conditioning designs;
(3)~it introduces a controlled diagnostic suite with planted trajectories that
isolates architectural effects from corpus confounds; (4)~it shows that a
global temporal signal reproduces aggregate drift in this regime, motivating
future comparison under noisier conditions; and (5)~it identifies
mean-prototype aggregation as a failure mode for bifurcating subjects.
...
the queried period. The central variant is Token-Time (joint token-period
projection); Standard is the null hypothesis; Additive (global period shift,
no token-level binding) is a secondary control; and Memory-Augmented extends
Token-Time with causal historical prototypes.
...
Token-Time vs.\ Standard isolates the effect of joint token-time conditioning.
Additive serves as a control for whether a global, token-agnostic temporal
signal suffices in this regime: it bounds how much of the conditioning gain
requires token-level binding rather than a uniform period shift.
Memory-Augmented vs.\ Token-Time tests the contribution of causal prototype
...
Token-Time conditioning moves drifting subjects substantially more than Standard
(\(\Delta\approx-0.563\) vs.\ \(-0.221\)), more than doubling the neighborhood
displacement. The Additive control reaches the same magnitude, establishing an
upper bound that does not require token-level binding in this high-fidelity
setting. Memory-Augmented does not improve over Token-Time.
...
encodes temporal information more precisely in harder conditions. Layer-wise
CKA is consistent with this: Additive vs.\ Token-Time representational
similarity increases from 0.726 to 0.887 (95\% CI [0.856, 0.919]) across
encoder depth, while Standard stays near 0.22, suggesting self-attention
partially absorbs the input-level difference, both pointing to the training
objective as the limiting factor.
...
vocabulary. The suite was co-designed with the architecture family, limiting its role as
an independent evaluation for models outside this design space.
