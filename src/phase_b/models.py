...
class _TemporalCrossAttention(nn.Module):
    ...
    def forward(self, h_subj: Tensor, memory: Tensor,
        ...
        if memory_mask is not None:
            ...
            # Para linhas sem histórico válido, desmascarar o primeiro slot (zero-vector)
            # para evitar que MHA receba all-True key_padding_mask e produza NaN.
            # O output dessas linhas será descartado depois.
            kpm = ~memory_mask                    # True onde IGNORAR (convenção PyTorch)
            needs_fix = ~has_valid                # linhas cujo key_padding_mask é all-True
            if needs_fix.any():
                kpm = kpm.clone()
                kpm[needs_fix, 0] = False         # força 1 key "válido" (zero-vector)
        ...
        updated = self.norm(h_subj + self.drop(attn_out))

        # Restaura linhas sem histórico real — o attention sobre zero-vector não deve
        # modificar a representação do sujeito
...
class B3(nn.Module):
    ...
    def embed(self, input_ids: Tensor, epoch_idx: Tensor) -> Tensor:
        positions = torch.arange(input_ids.size(1), device=input_ids.device)
        tok = self.token_emb(input_ids)
        t   = self.time_enc(epoch_idx)
        tok = self.tti(tok, t)
        return self.drop(tok + self.pos_emb(positions))

    def encode(self, input_ids: Tensor, epoch_idx: Tensor,
               memory: Tensor | None = None,
               memory_mask: Tensor | None = None) -> Tensor:
        """
        Retorna hidden states após atenção textual + atualização temporal.

        memory:      (batch, hist_len, d_model)  ou None para t0
        memory_mask: (batch, hist_len) bool — True onde válido
        """
        hidden = self.encoder(self.embed(input_ids, epoch_idx))  # (batch, seq, d_model)

        if memory is not None and memory.size(1) > 0:
            h_subj = hidden[:, POS_SUBJECT, :]                   # (batch, d_model)
            h_subj_updated = self.temp_attn(h_subj, memory, memory_mask)
            hidden = hidden.clone()
            hidden[:, POS_SUBJECT, :] = h_subj_updated
