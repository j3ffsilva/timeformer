...
compared with \(-0.221\) for Standard. Additive and Token-Time are not
distinguishable on drift score, and Memory-Augmented does not improve over
Token-Time.
...
performance points to the training objective as the limiting factor, not the
...
Two next steps follow. First, memory mechanisms should preserve the internal
structure of historical representations instead of collapsing each subject and
period to one prototype. Second, the convergence of Additive and Token-Time on
drift performance needs attention-level analysis: heads may suppress the
temporal signal during self-attention, neutralizing differences in input
conditioning. Period-stratified attention analysis would test this hypothesis
and clarify whether traceability needs changes to the objective, the
